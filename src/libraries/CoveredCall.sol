...
function computeSpotPrice(uint256 reserveBase_, uint256 totalLiquidity_, uint256 strike_, uint256 sigma_, uint256 tau_)
    pure
    returns (uint256)
{
    // Φ^-1(1 - x/L)
    int256 a = Gaussian.ppf(int256(1 ether - reserveBase_.divWadDown(totalLiquidity_)));
    // σ√τ
...
function computeY(uint256 reserveBase_, uint256 liquidity, uint256 strike_, uint256 sigma_, uint256 tau_)
    pure
    returns (uint256)
{
    int256 a = Gaussian.ppf(toInt(1 ether - reserveBase_.divWadDown(liquidity)));
...
function computeX(uint256 reserveQuote_, uint256 liquidity, uint256 strike_, uint256 sigma_, uint256 tau_)
    pure
    returns (uint256)
{
    int256 a = Gaussian.ppf(toInt(reserveQuote_ * 1e36 / (liquidity * strike_)));
    int256 b = tau_ != 0 ? toInt(computeSigmaSqrtTau(sigma_, tau_)) : int256(0);
    int256 c = Gaussian.cdf(a + b);
...
function computeL(uint256 reserveBase_, uint256 liquidity, uint256 sigma_, uint256 prevTau, uint256 newTau)
    pure
    returns (uint256)
{
    int256 a = Gaussian.ppf(toInt(reserveBase_ * 1 ether / liquidity));
    ...
    return reserveBase_ * 1 ether / toUint(1 ether - c);
}
...
function computeLGivenX(uint256 reserveBase_, uint256 S, uint256 strike_, uint256 sigma_, uint256 tau_)
    pure
    returns (uint256)
{
    ...
    return reserveBase_.divWadUp(1 ether - cdf);
}
...
function findX(bytes memory data, uint256 x) pure returns (int256) {
    (uint256 reserveQuote_, uint256 liquidity, uint256 strike_, uint256 sigma_, uint256 tau_) =
        abi.decode(data, (uint256, uint256, uint256, uint256, uint256));

    return computeTradingFunction(x, reserveQuote_, liquidity, strike_, sigma_, tau_);
}
...
function findY(bytes memory data, uint256 y) pure returns (int256) {
    (uint256 reserveBase_, uint256 liquidity, uint256 strike_, uint256 sigma_, uint256 tau_) =
        abi.decode(data, (uint256, uint256, uint256, uint256, uint256));

    return computeTradingFunction(reserveBase_, y, liquidity, strike_, sigma_, tau_);
}
...
function findL(bytes memory data, uint256 liquidity) pure returns (int256) {
    (uint256 reserveBase_, uint256 reserveQuote_, uint256 strike_, uint256 sigma_, uint256 tau_) =
        abi.decode(data, (uint256, uint256, uint256, uint256, uint256));

    return computeTradingFunction(reserveBase_, reserveQuote_, liquidity, strike_, sigma_, tau_);
}

/// todo: figure out what happens when result of trading function is negative or positive.
function solveX(uint256 reserveQuote_, uint256 liquidity, uint256 strike_, uint256 sigma_, uint256 tau_)
    pure
    returns (uint256 reserveBase_)
{
    bytes memory args = abi.encode(reserveQuote_, liquidity, strike_, sigma_, tau_);
    uint256 initialGuess = computeX(reserveQuote_, liquidity, strike_, sigma_, tau_);
    // at maturity the `initialGuess` will == L therefore we must reduce it by 1 wei
    reserveBase_ = findRootNewX(args, tau_ != 0 ? initialGuess : initialGuess - 1, 20, 10);
}

function solveY(uint256 reserveBase_, uint256 liquidity, uint256 strike_, uint256 sigma_, uint256 tau_)
    pure
    returns (uint256 reserveQuote_)
{
    bytes memory args = abi.encode(reserveBase_, liquidity, strike_, sigma_, tau_);
    uint256 initialGuess = computeY(reserveBase_, liquidity, strike_, sigma_, tau_);
    // at maturity the `initialGuess` will == LK (K == WAD, K*L == L) therefore we must reduce it by 1 wei
    reserveQuote_ = findRootNewY(args, tau_ != 0 ? initialGuess : initialGuess - 1, 20, 10);
}

function solveL(PoolPreCompute memory comp, uint256 initialLiquidity, uint256 reserveQuote_, uint256 sigma_)
    pure
    returns (uint256 liquidity_)
{
    bytes memory args = abi.encode(comp.reserveInAsset, reserveQuote_, comp.strike_, sigma_, comp.tau_);
    uint256 initialGuess =
        computeLGivenYK(comp.reserveInAsset, reserveQuote_, initialLiquidity, comp.strike_, sigma_, comp.tau_);
...
function computeDeltaLXIn(
    ...
    uint256 px = computeSpotPrice(reserveBase, totalLiquidity, strike, sigma, tau);
    deltaL = px.mulWadUp(totalLiquidity).mulWadUp(fees).divWadDown(px.mulWadDown(reserveBase) + reserveQuote);
}
...
function computeDeltaLYIn(
    ...
    uint256 fees = swapFee.mulWadUp(amountIn);
    uint256 px = computeSpotPrice(reserveBase, totalLiquidity, strike, sigma, tau);
    deltaL = totalLiquidity.mulWadUp(fees).divWadDown(px.mulWadDown(reserveBase) + reserveQuote);
}
    ...
    returns (uint256 reserveBase_)
{
    ...
    for (uint256 i = 0; i < maxIterations; i++) {
        ...
        reserveBase_next = int256(reserveBase_) - fx * 1e18 / dfx;

        if (abs(int256(reserveBase_) - reserveBase_next) <= int256(tolerance) || abs(fx) <= int256(tolerance)) {
            reserveBase_ = uint256(reserveBase_next);
            break;
        }
    ...
    returns (uint256 reserveQuote_)
{
    ...
    for (uint256 i = 0; i < maxIterations; i++) {
        ...
        reserveQuote_next = int256(reserveQuote_) - fx * 1e18 / dfx;

        if (abs(int256(reserveQuote_) - reserveQuote_next) <= int256(tolerance) || abs(fx) <= int256(tolerance)) {
            reserveQuote_ = uint256(reserveQuote_next);
            break;
        }
...
function calcMaxPtIn(
    uint256 reserveBase_,
        ...
        uint256 low = 0;
        uint256 high = reserveQuote_ - 1;

        while (low != high) {
            uint256 mid = (low + high + 1) / 2;
            if (calcSlope(reserveBase_, reserveQuote_, totalLiquidity_, strike_, int256(mid)) < 0) {
                high = mid - 1;
            } else {
                low = mid;
            }
...
function calcSlope(
    ...
    uint256 newReserveQuote = reserveQuote_ + uint256(ptToMarket);
    uint256 b_i = newReserveQuote * 1e36 / (strike_ * totalLiquidity_);
    ...
    int256 dxdy = computedXdY(reserveBase_, newReserveQuote);
...
function calcMaxPtOut(
    ...
    int256 currentTF = computeTradingFunction(reserveBase_, reserveQuote_, totalLiquidity_, strike_, sigma_, tau_);
    
    uint256 maxProportion = uint256(int256(1e18) - currentTF) * 1e18 / (2 * 1e18);
    
    uint256 maxPtOut = reserveQuote_ * maxProportion / 1e18;
...
function computedXdY(
    ...
    return -int256(reserveBase_) * 1e18 / int256(reserveQuote_);
}
